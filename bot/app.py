...
def send_welcome(message):
    if not str(message.chat.id) in joinedUsers:
        ...
        bot.send_message(message.from_user.id,
                         'Выберите нужный раздел: ', reply_markup=menu.start2)
    else:
        print("\nБот был запущен. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(message.chat.id, ""+config.name+"\n\nДобро пожаловать, "+message.chat.first_name +
...
def send_otziv(message):
    if message.text == '📦 Покупки':
        print('Пользователь нажал покупки. ID: '+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        ...
        if message.chat.id == config.admin_id:
            bot.send_message(config.admin_id, "☎️ Админ панель",
        ...
        if message.chat.id == config.admin_id:
            print('Пользователь нажал информация. ID: ' +
                  str(message.chat.id)+'    Дата/время: '+str(datetime.now()))
            ...
            print('Пользователь нажал кабинет. ID: ' +
                  str(message.chat.id)+'    Дата/время: '+str(datetime.now()))
            bot.send_message(message.chat.id, "Информация об аккаунте\n➖➖➖➖➖➖➖➖➖➖➖➖➖\n💎 Никнейм: "+message.chat.username +
        ...
        print('Пользователь нажал ассортимент. ID: ' +
              str(message.chat.id)+'    Дата/время: '+str(datetime.now()))
        bot.send_message(message.chat.id, "Выберите город:",
                         reply_markup=menu.city)
    elif message.text == '🔙Назад':
        if message.chat.id == config.admin_id:
            bot.send_message(
                config.admin_id, '🔙Вы вернулись в главное меню', reply_markup=menu.start2)
        else:
            bot.send_message(
                message.chat.id, '🔙Вы вернулись в главное меню', reply_markup=menu.start)
    elif message.text == 'Москва':
        print("Выбрал г. Москва. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_msk)
    elif message.text == 'Санкт-Петербург':
        print("Выбрал г. Санкт-Петербург. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_spb)
    elif message.text == 'Екатеринбург':
        print("Выбрал г. Екатеринбург. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_ekb)
    elif message.text == 'Домодедово':
        print("Выбрал г. Домодедово. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_msk)
    elif message.text == 'Мытищи':
        print("Выбрал г. Мытищи. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_spb)
    elif message.text == 'Зеленоград':
        print("Выбрал г. Зеленоград. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_perm)
    elif message.text == 'Подольск':
        print("Выбрал г. Подольск. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(message.chat.id, '⬇️ Какой район интересует? ⬇️',
                         reply_markup=menu.rayon_voronezh)
    elif message.text == 'Челябинск':
        print("Выбрал г. Челябинск. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(message.chat.id, '⬇️ Какой район интересует? ⬇️',
                         reply_markup=menu.rayon_voronezh)
    elif message.text == 'Нижний Новгород':
        print("Выбрал г. Нижний Новгород. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_spb)
    elif message.text == 'Иркутск':
        print("Выбрал г. Иркутск. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_msk)
    elif message.text == 'Оренбург':
        print("Выбрал г. Оренбург. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_perm)
    elif message.text == 'Пенза':
        print("Выбрал г. Пенза. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_spb)
    elif message.text == 'Омск':
        print("Выбрал г. Омск. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(message.chat.id, '⬇️ Какой район интересует? ⬇️',
                         reply_markup=menu.rayon_voronezh)
    elif message.text == 'Пермь':
        print("Выбрал г. Пермь. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_perm)
    elif message.text == 'Рязань':
        print("Выбрал г. Рязань. ID: "+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
        bot.send_message(
            message.chat.id, '⬇️ Какой район интересует? ⬇️', reply_markup=menu.rayon_msk)
    elif message.text in menu.rayon_spisok:
        print('Выбрал район '+message.text+'. ID: ' +
              str(message.chat.id)+'    Дата/время: '+str(datetime.now()))
        assort = bot.send_message(
            message.chat.id, '🧨 Ассортимент 🧨', reply_markup=menu.what)
...
def oplata(message):
    ...
    if message.text in menu.what_spisok:
        ...
        print('Выбрал товар: '+tovarka+'. ID: '+str(message.chat.id) +
              '    Дата/время: '+str(datetime.now()))
